...
public class Bank implements Runnable
{
    ...
    public static void main(String[] args)
    {
        ...
        System.out.println("Терминал банка. Вам доступны две команды: баланс {имя клиента банка}, перевод {имя клиента, у которого хотите снять деньги} {имя клиента, которому хотите передать сумму} {сумма перевода}");
        ...
        if (!(blockedAccounts.contains(fromAccountNum) || blockedAccounts.contains(toAccountNum)))
        {
            if (accounts.get(fromAccountNum).money >= amount && amount > 0)
            {
                if (amount <= 50000)
                {
                    ...
                    System.out.println("Перевод " + fromAccountNum + " -> " + toAccountNum + " = " + amount + " проведен успешно");
                    checkAllSum();
                }
                else
                {
                    if (isFraud(fromAccountNum, toAccountNum, amount))
                    {
                        ...
                        System.out.println("Перевод " + fromAccountNum + " -> " + toAccountNum + " = " + amount + " заблокирован. Подозрение в мошенничестве");
                    }
                    else
                    {
                        checkAllSum();
            ...
            {
                System.out.println("Перевод " + fromAccountNum + " -> " + toAccountNum + " = " + amount + " заблокирован. Недостаточно средств");
            }
        }
        else
        {
            System.out.println("Перевод " + fromAccountNum + " -> " + toAccountNum + " = " + amount + " заблокирован. Служба Безопасности не позволила провести перевод");
        }
    }
